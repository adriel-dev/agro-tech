...
class BreedRepositoryImpl(
    ...
    override fun updateBreed(breedId: UUID, breed: Breed): Breed {
        val foundBreed = breedJpaRepository.findById(breedId).orElseThrow { RuntimeException("Breed with id [$breedId] does not exist!") }
    ...
    override fun findBreedById(breedId: UUID): Breed {
        return breedJpaRepository.findById(breedId).get().toDomainBreed()
    }
